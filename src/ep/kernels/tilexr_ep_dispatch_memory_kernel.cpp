...
__aicore__ inline uint32_t MoeDistributeDispatchV2FullMesh<XType>::InitWinState(GlobalTensor<uint32_t> selfDataStatusGMTensor,
    ...
    TBuf<> dataStateBuf)
{
    // usedTime_[0] = AscendC::GetSystemCycle();
    ...
    // usedTime_[1] = AscendC::GetSystemCycle();
    ...
    // usedTime_[2] = AscendC::GetSystemCycle();
...
__aicore__ inline void MoeDistributeDispatchV2FullMesh<XType>::SetTilingDataAndCal()
{
    ...
    if (useMxfp8_) {
        ...
        hAlignSize_ = Ceil(axisH_, 128U) * 128U * sizeof(XType);
        ...
        quantWorkBytes_ = Ceil(
            Ceil(scaleOutBytes_, 32U) * 32U * sizeof(float) + scaleOutBytes_ * sizeof(uint16_t), UB_ALIGN) * UB_ALIGN;
    } else {
    ...
    int64_t expandXOutDtype, int64_t magic, TPipe *pipe)
{
    ...
    // usedTime_[3] = AscendC::GetSystemCycle();
}
    ...
    LocalTensor<uint8_t> &outLocal, LocalTensor<XType> &inLocal)
{
    ...
    __ubuf__ uint16_t *halfScaleAddr = reinterpret_cast<__ubuf__ uint16_t *>(
        quantWorkTensor_[Ceil(scaleOutBytes_, 32U) * 32U].GetPhyAddr());
    ...
    TileXRMxfp8Quant::ComputeScale<Fp8Type>(maxExpAddr, scaleAddr, halfScaleAddr, scaleOutBytes_);
    ...
    int32_t beginIdx, int32_t copyCnt)
{
    ...
    uint32_t compResultU64Num = Ceil(flagNum, 64);
