...
constexpr uint32_t DISPATCH_STATE_OFFSET = 768U * 1024U;
constexpr uint32_t STATE_SLOT_STRIDE_BYTES = 512U;
...
public:
    __aicore__ inline void Init(GM_ADDR commArgsGM, GM_ADDR expertOutGM, GM_ADDR assistInfoGM,
        ...
        int64_t activeMaskType, int64_t quantMode, TPipe *pipe);
    __aicore__ inline void Process();

private:
    __aicore__ inline void ReadDispatchStateAndSendCount();
...
__aicore__ inline void MoeDistributeCombineV2A5Mte<XType>::Init(GM_ADDR commArgsGM, GM_ADDR expertOutGM,
    ...
    int64_t globalBs, int64_t activeMaskType, int64_t quantMode, TPipe *pipe)
{
    ...
    inputAlignBytes_ = useMxfp8_ ? static_cast<uint32_t>(AlignUp(axisH_, 128U) * sizeof(XType)) : rowAlignBytes_;
    ...
    if (useMxfp8_) {
        ...
        quantWorkBytes_ = static_cast<uint32_t>(AlignUp(
            AlignUp(quantScaleCount_, 32U) * sizeof(float) +
                quantScaleCount_ * sizeof(uint16_t), UB_ALIGN));
    } else {
    ...
    ReadDispatchStateAndSendCount();
    SplitSendRows();
}

template <typename XType>
__aicore__ inline void MoeDistributeCombineV2A5Mte<XType>::ReadDispatchStateAndSendCount()
{
    LocalTensor<uint32_t> meta = metaBuf_.Get<uint32_t>();
    GlobalTensor<uint32_t> stateGlobal;
    stateGlobal.SetGlobalBuffer(reinterpret_cast<__gm__ uint32_t *>(context_.PeerDataBase(epRankId_) +
        DISPATCH_STATE_OFFSET + static_cast<uint64_t>(coreIdx_) * STATE_SLOT_STRIDE_BYTES));
    DataCopy(meta, stateGlobal, UB_ALIGN / sizeof(uint32_t));
    SyncFunc<AscendC::HardEvent::MTE2_S>();
    dataState_ = meta.GetValue(0) == 0U ? 1U : 0U;
    ...
    LocalTensor<uint8_t> quantLocal, LocalTensor<XType> inputLocal)
{
    ...
    __ubuf__ uint16_t *halfScaleAddr = reinterpret_cast<__ubuf__ uint16_t *>(
        workAddr + AlignUp(quantScaleCount_, 32U) * sizeof(float));
    ...
    TileXRMxfp8Quant::ComputeMaxExp(srcAddr, maxExpAddr, axisH_);
    TileXRMxfp8Quant::ComputeScale<Fp8Type>(
        maxExpAddr, scaleAddr, halfScaleAddr, quantScaleCount_);
    TileXRMxfp8Quant::ComputeFp8Data<XType, Fp8Type,
        RoundMode::CAST_TRUNC, RoundMode::CAST_RINT>(
        srcAddr, halfScaleAddr, outAddr, axisH_);
}
...
__aicore__ inline void RunCombine(GM_ADDR commArgsGM, GM_ADDR expertOutGM, GM_ADDR assistInfoGM,
    ...
    GM_ADDR yOutGM, int64_t bs, int64_t h, int64_t topK, int64_t moeExpertNum,
    int64_t sharedExpertNum, int64_t sharedExpertRankNum, int64_t globalBs,
    int64_t activeMaskType, int64_t quantMode)
{
    TPipe pipe;
    ...
    op.Init(commArgsGM, expertOutGM, assistInfoGM, sendCountsGM, expertScalesGM, xActiveMaskGM,
        ...
        globalBs, activeMaskType, quantMode, &pipe);
    op.Process();
}
...
extern "C" __global__ __aicore__ void tilexr_ep_combine_memory_kernel(GM_ADDR commArgsGM, GM_ADDR expertOutGM,
    ...
    int64_t activeMaskType, int64_t quantMode, int64_t dtype)
{
    if (commArgsGM == nullptr || expertOutGM == nullptr || assistInfoForCombineGM == nullptr ||
        ...
        moeExpertNum <= 0 || sharedExpertNum < 0 || sharedExpertRankNum < 0 || globalBs <= 0 ||
    ...
    if (dtype == static_cast<int64_t>(TileXR::TILEXR_DATA_TYPE_FP16)) {
        Mc2Kernel::RunCombine<half>(commArgsGM, expertOutGM, assistInfoForCombineGM, sendCountsGM,
            ...
            sharedExpertNum, sharedExpertRankNum, globalBs, activeMaskType, quantMode);
...
void launch_tilexr_ep_combine_memory_kernel(uint32_t blockDim, void *stream, GM_ADDR commArgs,
    GM_ADDR expertOut, GM_ADDR assistInfoForCombine, GM_ADDR sendCounts, GM_ADDR expertScales,
    GM_ADDR xActiveMask, GM_ADDR sharedExpertX, GM_ADDR yOut, int64_t bs, int64_t h, int64_t topK,
    int64_t moeExpertNum, int64_t sharedExpertNum, int64_t sharedExpertRankNum, int64_t globalBs,
    int64_t activeMaskType, int64_t quantMode, int64_t dtype)
{
    tilexr_ep_combine_memory_kernel<<<blockDim, nullptr, stream>>>(commArgs, expertOut, assistInfoForCombine,
        sendCounts, expertScales, xActiveMask, sharedExpertX, yOut, bs, h, topK, moeExpertNum,
        sharedExpertNum, sharedExpertRankNum, globalBs, activeMaskType, quantMode, dtype);
}
