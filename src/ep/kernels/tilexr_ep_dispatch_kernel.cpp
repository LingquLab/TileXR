    ...
    int64_t payloadBytesPerSlot, int64_t assistBytesPerSlot, int64_t slotBytes, int64_t totalBytes, int64_t magic)
{
    if constexpr (g_coreType == AscendC::AIV) {
        ...
        const int32_t rankSize = args->rankSize;
        if (rankSize <= 0 || rankSize > TileXR::TILEXR_MAX_RANK_SIZE || rank < 0 || rank >= rankSize ||
            !IsValidShape(bs, h, topK, moeExpertNum, dtypeBytes, maxRoutesPerSrc, rowBytes, rowBytes,
                payloadBytesPerSlot, assistBytesPerSlot, slotBytes, totalBytes, rankSize, rankSize, 0, 0,
                kEpQuantModeNone) ||
            dtype != kTileXrDataTypeFp16) {
            return;
        }
        ...
        ScatterCombineRows(localWindow, expertOutGM, assistInfoForCombineGM, epRecvCountsGM, rankSize, maxRoutesPerSrc,
            rowBytes, slotBytes, payloadBytesPerSlot, tBuf);
        AscendC::PipeBarrier<PIPE_ALL>();
...
void launch_tilexr_ep_combine_kernel(uint32_t blockDim, void *stream, GM_ADDR commArgs, GM_ADDR expertOut,
    ...
    int64_t payloadBytesPerSlot, int64_t assistBytesPerSlot, int64_t slotBytes, int64_t totalBytes, int64_t magic)
{
    tilexr_ep_combine_kernel<<<blockDim, nullptr, stream>>>(commArgs, expertOut, assistInfoForCombine, epRecvCounts, yOut,
        bs, h, topK, moeExpertNum, dtype, dtypeBytes, maxRoutesPerSrc, rowBytes, payloadBytesPerSlot, assistBytesPerSlot,
        slotBytes, totalBytes, magic);
}
