...
extern void launch_tilexr_ep_combine_memory_kernel(uint32_t blockDim, void *stream, GM_ADDR commArgs,
...
int TileXREpLaunchCombineMemoryKernel(const EpCombineParams &params, const EpHostLaunchContext &context)
{
    ...
    const int64_t globalBs = params.globalBs == 0 ? params.bs * context.hostArgs->rankSize : params.globalBs;
    launch_tilexr_ep_combine_memory_kernel(blockDim, params.stream, context.devArgs,
        ...
        params.quantMode, static_cast<int64_t>(params.dtype));
    return TileXR::TILEXR_SUCCESS;
}
