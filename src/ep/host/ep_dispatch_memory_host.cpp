    ...
    EpWindowConfig *window)
{
    ...
    const int ret = TileXREpValidateDispatchMemoryConfig(
        params, commArgs, kEpMemoryDefaultVectorCoreNum, &memoryConfig);
    if (ret != TileXR::TILEXR_SUCCESS) {
        return ret;
    }

    EpWindowConfig compatible {};
    compatible.rankSize = commArgs.rankSize;
    compatible.bs = params.bs;
    compatible.h = params.h;
    compatible.topK = params.topK;
    compatible.moeExpertNum = params.moeExpertNum;
    compatible.localExpertNum = memoryConfig.localExpertNum;
    compatible.dtypeBytes = params.quantMode == 4 ? 1 : TileXREpDataTypeSize(params.dtype);
    compatible.maxRoutesPerSrc = params.bs * (params.topK + params.sharedExpertNum);
    compatible.rowBytes = params.h * compatible.dtypeBytes;
    compatible.payloadRowBytes = compatible.rowBytes + memoryConfig.scaleOutBytes;
    compatible.totalBytes = memoryConfig.totalWinSize + kEpMemoryStateWindowBytes + memoryConfig.workspaceBytes;
    *window = compatible;
    return TileXR::TILEXR_SUCCESS;
}
