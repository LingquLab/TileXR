...
bool ValidatePlan(const TileXRMoonEpPlanV1 *plan)
{
    if (plan == nullptr || plan->structSize < sizeof(*plan) ||
        plan->abiVersion != TILEXR_MOONEP_ABI_VERSION_V1 || plan->s <= 0 ||
        plan->k <= 0 || plan->e <= 0 || plan->b <= 0 || plan->rank < 0 ||
        plan->world <= 0 || plan->world > kReduceGradMaxAivBlockCount ||
        plan->rank >= plan->world || plan->e % plan->world != 0 ||
        plan->b != plan->e / plan->world || plan->expertsToCopy == nullptr) {
        return false;
    }
    uint64_t capacity = 0;
    return CheckedMul(static_cast<uint64_t>(plan->s), static_cast<uint64_t>(plan->k),
        &capacity) && capacity <= static_cast<uint64_t>(INT64_MAX) &&
        plan->dispatchedCapacity == static_cast<int64_t>(capacity);
}
    ...
    const TileXR::TileXRUDMARegistry **registryOut)
{
    ...
    *registryOut = nullptr;
    ...
    if (params.workspace == nullptr || params.workspaceBytes < layout.workspaceBytes ||
        reinterpret_cast<uintptr_t>(params.workspace) % kReduceGradUdmaAlignment != 0 ||
    ...
    for (int32_t rank = 0; rank < commArgs.rankSize; ++rank) {
        if (!TileXR::UDMARegionContains(registry, rank, 0, layout.workspaceBytes)) {
            return TileXR::TILEXR_ERROR_PARA_CHECK_FAIL;
        }
    }
    ...
    TileXRMoonEpReduceGradWorkspaceInfoV2 *info)
{
    ...
    info->workspaceAlignment = kReduceGradUdmaAlignment;
...
{
    if (comm == nullptr || !ValidatePlan(plan) || gradients == nullptr || layout == nullptr) {
        return TileXR::TILEXR_ERROR_PARA_CHECK_FAIL;
    }
    ...
    if (!LocalityValid(*commArgs) || plan->rank != commArgs->rank ||
        plan->world != commArgs->rankSize) {
        return TileXR::TILEXR_ERROR_PARA_CHECK_FAIL;
    }
    ...
    return TileXRMoonEpBuildReduceGradLayout(plan->rank, plan->world, plan->e,
    ...
    aclrtStream stream)
{
    ...
    return TileXRMoonEp::TileXRMoonEpLaunchReduceGradKernel(params, context);
}
