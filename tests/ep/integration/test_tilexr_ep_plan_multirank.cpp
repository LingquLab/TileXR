    ...
    ControlRecord *finalRecord)
{
    ...
    if (valid) {
        ...
        if (rankSize > 1 && legacyActualStatus.size() > 2 && legacyActualStatus[2] <= 0) {
            std::cerr << "rank " << rank << " legacy validation did not exercise an inter-server round"
                      << std::endl;
        }
        if (rankSize > 1 && actualStatus.size() > 2 && actualStatus[2] <= 0) {
...
int main(int argc, char **argv)
{
    ...
    if ((rankSize != 1 && rankSize != 2 && rankSize != 8 && rankSize != 32 && rankSize != 128) ||
        rank < 0 || rank >= rankSize ||
        deviceId < 0 || deviceId >= 8) {
        std::cerr << "usage: test_tilexr_ep_plan_multirank <1|2|8|32|128> <rank> <device 0..7>" << std::endl;
        return 2;
    }
