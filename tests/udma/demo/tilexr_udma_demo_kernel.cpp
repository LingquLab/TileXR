    ...
    uint32_t bytes, uint32_t pattern, int32_t traffic)
{
    ...
    int32_t peer = -1;
    if (!enabled || blockNum == 0 || !TileXRUdmaDemoResolvePeer(rank, srcRank, dstRank, traffic, peer)) {
        return;
    }
    if (blockIdx != 0) {
    ...
    TileXR::UDMAPutNbi<uint8_t>(args, peer, src, dstByteOffset, bytes);
...
extern "C" __global__ __aicore__ void tilexr_udma_p2p_perf_multi_wqe_kernel(
    GM_ADDR commArgsGM, GM_ADDR srcGM, GM_ADDR debugGM,
    int32_t srcRank, int32_t dstRank, uint64_t dstByteOffset,
    uint32_t bytes, uint32_t pattern, int32_t traffic)
{
    ...
    for (uint32_t i = 0; i < blockNum; ++i) {
        ...
        TileXRUdmaDemoWqeSlice(bytes, blockNum, i, offset, sliceBytes);
        if (sliceBytes == 0) {
            continue;
        }
        TileXR::UDMAPutNbi<uint8_t>(args, peer, src + offset, dstByteOffset + offset, sliceBytes);
        ++posted;
    }
    uint32_t status = posted == 0 ? 0 : TileXR::UDMAQuietStatus(args, peer);
    TileXRUdmaDemoFoldDebugStatus(debug, blockIdx, status);
    if (debug != nullptr) {
        debug[5] = status;
        debug[7] = posted;
...
extern "C" __global__ __aicore__ void tilexr_udma_p2p_perf_multi_jetty_kernel(
    GM_ADDR commArgsGM, GM_ADDR srcGM, GM_ADDR debugGM,
    int32_t srcRank, int32_t dstRank, uint64_t dstByteOffset,
    uint32_t bytes, uint32_t pattern, int32_t traffic)
{
    ...
    for (uint32_t qpIdx = 0; qpIdx < jettyCount; ++qpIdx) {
        uint32_t offset = 0;
        uint32_t sliceBytes = 0;
        TileXRUdmaDemoWqeSlice(bytes, jettyCount, qpIdx, offset, sliceBytes);
        if (debug != nullptr && qpIdx < 8) {
            debug[16 + qpIdx] = offset;
            debug[24 + qpIdx] = sliceBytes;
        }
        if (sliceBytes == 0) {
            continue;
        }
        TileXR::UDMAPutNbiQp<uint8_t>(args, peer, qpIdx, src + offset, dstByteOffset + offset, sliceBytes);
        ++posted;
    }

    uint32_t status = 0;
    for (uint32_t qpIdx = 0; qpIdx < posted; ++qpIdx) {
        status |= TileXR::UDMAQuietStatusQp(args, peer, qpIdx);
    }
    TileXRUdmaDemoFoldDebugStatus(debug, blockIdx, status);
    if (debug != nullptr) {
        debug[5] = status;
...
extern "C" __global__ __aicore__ void tilexr_udma_p2p_perf_multi_jetty_parallel_kernel(
    GM_ADDR commArgsGM, GM_ADDR srcGM, GM_ADDR debugGM,
    int32_t srcRank, int32_t dstRank, uint64_t dstByteOffset,
    uint32_t bytes, uint32_t pattern, int32_t traffic)
{
...
extern "C" __global__ __aicore__ void tilexr_udma_p2p_perf_multi_jetty_parallel_fixed_wqe_kernel(
    GM_ADDR commArgsGM, GM_ADDR srcGM, GM_ADDR debugGM,
    int32_t srcRank, int32_t dstRank, uint64_t dstByteOffset,
    uint32_t bytes, uint32_t pattern, int32_t traffic)
{
    ...
    uint64_t stride = ((static_cast<uint64_t>(bytes) + kFixedWqeAddressAlignment - 1ULL) /
        kFixedWqeAddressAlignment) * kFixedWqeAddressAlignment;
    uint64_t offset = static_cast<uint64_t>(blockIdx) * stride;
    if (debug != nullptr && blockIdx < 8) {
        debug[16 + blockIdx] = static_cast<uint32_t>(offset & 0xffffffffU);
        debug[24 + blockIdx] = bytes;
    }
    if (bytes == 0) {
        TileXRUdmaDemoFoldDebugStatus(debug, blockIdx, 0);
        return;
    }
    TileXR::UDMAPutNbiQp<uint8_t>(args, peer, blockIdx, src + offset, dstByteOffset + offset, bytes);
    uint32_t status = TileXR::UDMAQuietStatusQp(args, peer, blockIdx);
    TileXRUdmaDemoFoldDebugStatus(debug, blockIdx, status);
    if (debug != nullptr && blockIdx == 0) {
        debug[5] = status;
...
void launch_tilexr_udma_p2p_perf_multi_wqe(
    uint32_t blockDim, void* stream, GM_ADDR commArgs, GM_ADDR src, GM_ADDR debug,
    int32_t srcRank, int32_t dstRank, uint64_t dstByteOffset, uint32_t bytes, uint32_t pattern, int32_t traffic)
{
    tilexr_udma_p2p_perf_multi_wqe_kernel<<<blockDim, nullptr, stream>>>(
        commArgs, src, debug, srcRank, dstRank, dstByteOffset, bytes, pattern, traffic);
}

void launch_tilexr_udma_p2p_perf_multi_jetty(
    uint32_t blockDim, void* stream, GM_ADDR commArgs, GM_ADDR src, GM_ADDR debug,
    int32_t srcRank, int32_t dstRank, uint64_t dstByteOffset, uint32_t bytes, uint32_t pattern, int32_t traffic)
{
    tilexr_udma_p2p_perf_multi_jetty_kernel<<<blockDim, nullptr, stream>>>(
        commArgs, src, debug, srcRank, dstRank, dstByteOffset, bytes, pattern, traffic);
}

void launch_tilexr_udma_p2p_perf_multi_jetty_parallel(
    uint32_t blockDim, void* stream, GM_ADDR commArgs, GM_ADDR src, GM_ADDR debug,
    int32_t srcRank, int32_t dstRank, uint64_t dstByteOffset, uint32_t bytes, uint32_t pattern, int32_t traffic)
{
    tilexr_udma_p2p_perf_multi_jetty_parallel_kernel<<<blockDim, nullptr, stream>>>(
        commArgs, src, debug, srcRank, dstRank, dstByteOffset, bytes, pattern, traffic);
}

void launch_tilexr_udma_p2p_perf_multi_jetty_parallel_fixed_wqe(
    uint32_t blockDim, void* stream, GM_ADDR commArgs, GM_ADDR src, GM_ADDR debug,
    int32_t srcRank, int32_t dstRank, uint64_t dstByteOffset, uint32_t bytes, uint32_t pattern, int32_t traffic)
{
    tilexr_udma_p2p_perf_multi_jetty_parallel_fixed_wqe_kernel<<<blockDim, nullptr, stream>>>(
        commArgs, src, debug, srcRank, dstRank, dstByteOffset, bytes, pattern, traffic);
}
