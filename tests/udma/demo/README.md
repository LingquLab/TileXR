...
bash demo/run_tilexr_udma_p2p_concurrency_sweep.sh 4096 16777216 2 20 5 0 1 direct_urma,memory,memory_visible_ack,memory_visible_ack_perf,data_as_flag unidir,bidir 1,2,4,8
```
...
The P2P performance scripts expose five user-facing transports:
`direct_urma`, `memory`, `memory_visible_ack`, `memory_visible_ack_perf`, and `data_as_flag`. `direct_urma`
...
destination buffer. `memory_visible_ack_perf` writes the same tail flag after the
payload and verifies it on the host after the measured window, keeping the CSV
timing focused on sender-side write-path performance.
