...
export TILEXR_DEMO_ALLTOALL_REPEAT=100
...
echo "=== rank tails ==="
for rank in $(seq 0 $((RANK_SIZE-1))); do echo "--- rank ${rank} ---"; tail -4 /tmp/a2a_bw_rank${rank}.log 2>/dev/null; done
exit ${ret}