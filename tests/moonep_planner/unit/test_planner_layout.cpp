...
void TestPrimary()
{
    ...
    const int ret = TileXRMoonEp::TileXRMoonEpBuildPlannerLayout(8, 8192, 16, 896, &layout);
    ...
    Check(layout.dispatchedCapacity == layout.routeCount, "primary NvS must equal N");
...
void TestBlockDimOverride()
{
    ...
    Check(TileXRMoonEp::TileXRMoonEpBuildPlannerLayout(16, 64, 4, 64, &layout) != 0,
...
void TestEncodedCapacityBoundary()
{
    ...
    Check(TileXRMoonEp::TileXRMoonEpBuildPlannerLayout(
        64, kExactlyRepresentableRoutes, 1, 64, &layout) == TileXR::TILEXR_SUCCESS,
        "R*N == INT32_MAX+1 rejected");
    Check(layout.dispatchedCapacity == kExactlyRepresentableRoutes,
        "boundary dispatched capacity mismatch");
    Check(TileXRMoonEp::TileXRMoonEpBuildPlannerLayout(
        64, kExactlyRepresentableRoutes + 1, 1, 64, &layout) != 0,
...
{
    TileXRMoonEp::PlannerLayout layout {};
    Check(TileXRMoonEp::TileXRMoonEpBuildPlannerLayout(0, 1, 1, 1, &layout) != 0,
        "zero rank accepted");
    Check(TileXRMoonEp::TileXRMoonEpBuildPlannerLayout(8, 1, 33, 896, &layout) != 0,
        "K > 32 accepted");
    Check(TileXRMoonEp::TileXRMoonEpBuildPlannerLayout(8, 1, 1, 895, &layout) != 0,
        "non-divisible experts accepted");
    Check(TileXRMoonEp::TileXRMoonEpBuildPlannerLayout(8, 1, 1, 8192, &layout) != 0,
    ...
    Check(TileXRMoonEp::TileXRMoonEpBuildPlannerLayout(8, std::numeric_limits<int32_t>::max(),
        32, 896, &layout) != 0, "overflow route count accepted");
}
