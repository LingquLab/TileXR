...
5. Define flattened peer sender/receiver work, one QP-0 control owner per peer,
   UDMA sequence/ack work, and final-tail bounds per projection.
...
- UDMA projections stage source chunks through the registered workspace and use
  `UDMAPutRegisteredSignalNbi`; they never target `peerMems` or unregistered
  remote memory.
- Exactly one persistent control AIV owns QP 0 submissions for each target
  peer. No producer or receiver AIV may mutate that peer's WQ state.
...
- Multiple AIVs never race a peer's QP 0, UDMA stage reuse requires the exact
...
- UDMA uses only registered workspace offsets and one QP-0 control AIV per peer;
  SDMA and HCCL remain absent.
