...
writer of all negotiated QPs for that peer and performs outgoing data
submissions plus completion GET polling. The selected QP is
`(sequenceOrdinal - 1) % udmaQpCount`; the QP-aware registered-memory helpers
update that QP's SQ state. Receiver AIVs consume local inbound stages and write
local completion state, but never post WQEs.
...
1. the peer control AIV copies source bytes through UB into its registered local
   ...
   `UDMAPutRegisteredSignalNbiOnQp` to copy the payload to the owner's inbound
...
3. a receiver AIV bounded-polls ready, invalidates the inbound payload cache
   ...
   line;
4. the source control AIV uses checked UDMA GETs on the same selected target QP to poll
...
- Source checks requiring Runtime V2 launch in Host, PR #90 DataAsFlag format,
  ...
  code in the kernel, no HCCL dependency, one control AIV per peer rotating over
  all negotiated QPs, and checked UDMA completion.
