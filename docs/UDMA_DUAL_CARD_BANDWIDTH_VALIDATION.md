...
The first concurrent run reached `97.33%` of the physical `800 GB/s` ceiling and retained `99.94%` of the summed sequential baseline. The result is approximately twice the average single-card bandwidth. Do not add rank-0 and rank-1 values from one pair: the demo reports each direction separately, and doing so would double-count the same full-duplex link capacity.
...
5. Reserve two communicator ports and their demo barrier ports. The barrier is `TILEXR_COMM_ID + 97`; this procedure uses `12000/12097` and `12200/12297`.
...
```text
TILEXR_UDMA_PERF rank=0 device=1 qp_count=4 bytes_per_iter=268435456 iterations=4000 elapsed_ms=... tx_GBps=... wall_elapsed_ms=... wall_tx_GBps=...
```
...
bash demo/run_tilexr_udma_data_channel_probe_mpi.sh \
  "${PERF_ARGS[@]}" \
  --comm-id 141.61.49.195:12000 \
  --devices 1,1 >pair_a.log 2>&1 &
pid_a=$!

bash demo/run_tilexr_udma_data_channel_probe_mpi.sh \
  "${PERF_ARGS[@]}" \
  --comm-id 141.61.49.195:12200 \
  --devices 6,5 >pair_b.log 2>&1 &
pid_b=$!
