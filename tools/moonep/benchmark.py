...
def _planning_launch_barrier(buffer, case_id: str, epoch: int) -> None:
    if os.environ.get("TILEXR_MOONEP_MANAGED_LAUNCH", "0") != "1":
        return
...
def make_inputs(torch_module, case, context):
    ...
    route_begin = context.planner_group_rank * route_count
    route_ids = torch_module.tensor(
        all_topk[route_begin : route_begin + route_count],
...
def validate_plan(plan, context, *, route_pattern: str = "balanced") -> dict[str, object]:
...
def reduce_grad_traffic(plan, context, info) -> dict[str, object]:
    ...
    for slot in range(context.experts_per_rank):
        ...
        if owner >= 0 and owner != context.planner_group_rank:
            for row_bytes, transport in zip(info.row_bytes, info.transports):
                ...
                total += row_bytes
                by_transport[transport] += row_bytes
    ...
    return {
        ...
        "transport_transferred_bytes": by_transport,
        "cross_node_transferred_bytes": cross_node,
    }
...
def run_case(torch_module, case, args, root: Path) -> None:
    ...
    try:
        ...
        def coordinated_iteration(timer=None):
            ...
            _planning_launch_barrier(buffer, case.case_id, planning_epoch)
            planning_epoch += 1
        ...
        if case.correctness:
            ...
            validation = validate_plan(
                first[0], context, route_pattern=case.route_pattern
            )
        ...
        failure = (exc, exc.__traceback__)
        print(
            f"[rank {global_rank}] {case.case_id} failed: "
