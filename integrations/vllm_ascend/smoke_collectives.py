...
from tilexr_collectives import all_gather, all_reduce, all_to_all, broadcast, reduce_scatter
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--op",
        choices=["allgather", "alltoall", "allreduce", "reducescatter", "broadcast"],
        default="allgather",
    )
...
def check_all_gather(result, rank_size: int, count: int, dtype) -> None:
    cpu = result.cpu().reshape(rank_size, count)
...
def make_all_to_all_values(rank_size: int, rank: int, count: int, dtype):
    ...
    for dst in range(rank_size):
        for idx in range(count):
            host.append(rank * 1000 + dst * 100 + idx)
...
def check_all_to_all(result, rank_size: int, rank: int, count: int, dtype) -> None:
    ...
    for src in range(rank_size):
        for idx in range(count):
            expected_values.append(src * 1000 + rank * 100 + idx)
...
def check_all_reduce(result, rank_size: int, count: int, dtype) -> None:
    cpu = result.cpu().reshape(-1)
    expected = torch.zeros(count, dtype=torch.int32)
    for src in range(rank_size):
        expected += torch.arange(src * 1000, src * 1000 + count, dtype=torch.int32)
    if dtype is torch.float16:
        expected = expected.to(torch.float16)
...
def check_reduce_scatter(result, rank_size: int, rank: int, count: int, dtype) -> None:
    ...
    expected = torch.tensor(
        [sum(src * 1000 + rank * 100 + idx for src in range(rank_size)) for idx in range(count)],
        dtype=torch.int32,
    )
    if dtype is torch.float16:
        expected = expected.to(torch.float16)
