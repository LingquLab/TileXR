...
class TileXRMoonEPBuffer:
    ...
    def prepare_reduce_grad(self, plan: MoonEPPlan, gradients: ProjectionBuffers):
        self._require_open()
        self._validate_plan(plan)
        ...
        if info.uses_udma:
            if not self._quiesced:
                self.synchronize()
            workspace = self._zeros((info.workspace_bytes,), self._torch.uint8)
            if int(workspace.data_ptr()) % int(info.workspace_alignment) != 0:
                raise RuntimeError(
                    "NPU allocator did not satisfy ReduceGrad UDMA workspace alignment"
                )
            self.quiesce()
            self.runtime.register_reduce_grad_workspace(workspace, info.workspace_bytes)
    ...
    def reduce_grad(self, plan: MoonEPPlan, gradients: ProjectionBuffers) -> None:
        info = self.prepare_reduce_grad(plan, gradients)
